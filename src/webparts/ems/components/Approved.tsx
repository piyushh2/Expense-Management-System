...
const Approved: React.FC<IApprovalsProps> = (props) => {
  ...
  const getNextManagerEmail = async () => {
    try {
      const listUrl = `${props.siteUrl}/_api/web/lists/getbytitle('Employee')/items`;
      const listResponse: SPHttpClientResponse = await props.context.spHttpClient.get(
        listUrl,
      ...
      const res = await listResponse.json();
      const filteredEmployee = res.value.filter((item: any) => item.Email === userEmail);
      setNextManager(filteredEmployee[0].ManagerEmail);
    }
    catch (error) {
      console.log(`Error: ${error}`);
    }
  ...
  useEffect(() => {
    ...
    void getNextManagerEmail();
    void getExpenseData();
  }, [props.siteUrl, props.context, refreshTrigger]);
  const userExpenses = ExpenseData.filter((item: any) =>
    (item.ManagerEmail === nextManager && item.Email !== userEmail) || (item.ManagerEmail == userEmail && (item.Status === "Approved" || item.Status === "Rejected"))
  );
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "auto", px: 2, mb: 2 }}>
      {selectedMenu === "New" && <New selectedRow={selectedRow} view={view} setSelectedMenu={setSelectedMenu} siteUrl={props.siteUrl} context={props.context} EmployeeDetails={EmployeeDetails} isManager={isManager} neww={false} edit={false} viewTable={false} setViewTable={undefined} setNeww={undefined} setEdit={undefined} />}
      {selectedMenu !== "New" &&
        <Box sx={{ display: "flex", flexGrow: 1, flexDirection: "column", alignItems: "center", mt: 2 }}>
          <Box sx={{ width: "fit-content", minWidth: "97%" }}>
            <DataGrid
              rows={approvalsRows}
